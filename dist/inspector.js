...
/* eslint-disable
    id-length,
*/
class RicoInspector extends HTMLElement {
  connectedCallback() {
    this.editorElement = document.querySelector("rico-editor[rico-id='".concat(this.dataset.ricoId, "']"));
...
window.JST["rico/inspector/templates/render"] = () => "Syncs: ".concat(window.syncCount);
...
class View {
  ...
  render() {
    ...
    if (this.isOpen()) {
      this.panelElement.innerHTML = window.JST["rico/inspector/templates/".concat(this.constructor.template)].apply(this);
    }
  }
  ...
  getSettingsKey(key) {
    return "rico/inspector/".concat(this.template, "/").concat(key);
  }
